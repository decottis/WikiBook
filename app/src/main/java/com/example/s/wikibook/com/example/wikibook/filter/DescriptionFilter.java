package com.example.s.wikibook.com.example.wikibook.filter;

import com.example.s.wikibook.Book;

/**
 * Created by Geoffrey on 06/10/2015.
 */
public class DescriptionFilter extends BookFilter {
    public DescriptionFilter(String toSearch){
        super(toSearch);
    }

    @Override
    public boolean isSelected(Book book) {
        if(book.getDescription().toLowerCase().contains(toSearch.toLowerCase())){
            return true;
        }
        return false;
    }
}
